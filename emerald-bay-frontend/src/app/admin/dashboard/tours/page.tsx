...
import { API_BASE_URL, TOURS_API, apiFetch } from "@/lib/api";
...
const normalizeTourImageUrl = (image?: string | null): string => {
  if (!image) return "";
...
export default function ToursPage() {
  ...
  useEffect(() => {
    const fetchTours = async () => {
      ...
      try {
        ...
        const data = await apiFetch(TOURS_API.LIST, { token });
        const toursArray = Array.isArray(data) ? data : data?.data || [];
        setTours(toursArray.map(normalizeTour));
        setApiError("");
      } catch (err) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const response = await apiFetch(TOURS_API.CREATE, {
        method: "POST",
        data: tourData,
        token,
      });

      const createdTour = response?.data ?? response;

      if (createdTour && createdTour.id) {
        setTours((prev) => [...prev, normalizeTour(createdTour)]);
        showSuccess("Tour created successfully!");
        resetForm();
      } else {
        setApiError("Failed to create tour");
      }
    } catch (error) {
      console.error("Create error:", error);
      setApiError("Failed to create tour. Please try again.");
    }
  };

  const handleUpdate = async () => {
    if (!editingTour || !token) {
      setApiError("Authentication required");
      return;
    }
    ...
    try {
      ...
      if (updatedTour && updatedTour.id) {
        ...
        setApiError("Failed to update tour");
      }
    } catch (error) {
      console.error("Update error:", error);
      setApiError("Failed to update tour. Please try again.");
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await apiFetch(TOURS_API.DELETE(id), {
        method: "DELETE",
        token,
      });
      ...
      setApiError("Failed to delete tour. Please try again.");
    }
  };
