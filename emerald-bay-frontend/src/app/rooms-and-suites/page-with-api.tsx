...
// Keep fallback rooms data for display if API fails
...
export default function RoomsAndSuitesPage() {
    ...
    useEffect(() => {
        const fetchRooms = async () => {
            try {
                setLoading(true);
                const data = await apiFetch(ROOMS_API.LIST);
                // Assuming API returns { data: [...] } or direct array
                const roomsData = Array.isArray(data) ? data : data?.data || data;
                setRooms(roomsData);
                setError(null);
            } catch (err) {
